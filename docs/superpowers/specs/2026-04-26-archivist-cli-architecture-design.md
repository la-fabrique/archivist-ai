# archivist-cli — Vision d'architecture

**Date :** 2026-04-26
**Statut :** vision haute, validée pour écriture
**Audience :** contributeurs présents et futurs du package `archivist-cli`

> Ce document fixe la vision d'architecture de `archivist-cli` à un niveau haut : périmètre, pipeline, ports, règles de dépendance, évolutivité. Il n'entre pas dans le détail des signatures, des erreurs ou des choix d'implémentation. Ces points seront couverts par les plans d'implémentation et, plus tard, par des ADRs ciblés.
